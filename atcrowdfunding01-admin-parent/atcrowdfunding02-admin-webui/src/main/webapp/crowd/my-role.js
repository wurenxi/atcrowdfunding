...
function fillAuthTree(){
    ...
    // 从服务器端查询到的list不需要组装树形结构，这里我们交给zTree去组装
    ...
    for (let i = 0; i < authIdArray.length; i++) {
        ...
        let checked = true;
        // chekTypeFlag设置为false表示不"联动"，不联动是为了避免把不应该勾选的勾选上
        let chekTypeFlag = false;
        zTreeObj.checkNode(treeNode,checked,chekTypeFlag);
    }
}
...
function getPageInfoRemote(){
    ...
    // 如果当前响应状态码不是200，说明发生了错误或其他意外情况，显示提示信息，让当前函数停止执行
    if(statusCode != 200){
...
function fillTableBody(pageInfo){
    ...
    for (let i = 0; i < pageInfo.list.length; i++) {
        ...
        let pencilBtn = "<button id='"+roleId+"' type='button' class='pencilBtn btn btn-primary btn-xs'><i class=' glyphicon glyphicon-pencil'></i></button>";

        let removeBtn = "<button id='"+roleId+"' type='button' class='removeBtn btn btn-danger btn-xs'><i class=' glyphicon glyphicon-remove'></i></button>";
